...
class AtomConcentraionCalculator:
    ...
    def calculate(
        ...
        intensity_ratios = self._calculate_intensity_ratios(
            first_species_atomic_lines, first_species_integrals, second_species_atomic_lines, second_species_integrals)
    ...
    def _calculate_intensity_ratios(self, first_species_atomic_lines, first_species_integrals, second_species_atomic_lines, second_species_integrals):
        ...
        e_values = second_species_atomic_lines[:, 3] - \
            first_species_atomic_lines[:, 3][:, np.newaxis]

        return np.stack(
            (e_values.flatten(), ln_ratios.flatten()), axis=-1)

    def _fit_intensity_ratios(self, intensity_ratios):
        return np.polyfit(
            intensity_ratios[:, 0], intensity_ratios[:, 1], 1)

    def _caculate_temperature(self, fitted_ratios):
        return 1/(0.695035*fitted_ratios[0])

    def _calculate_atom_concentration_ratio(self, fitted_ratios, temperature, first_species_name, second_species_name):
        partition_function_first_species = self._partition_function_data_getter.get_data(
            first_species_name, temperature)
        partition_function_second_species = self._partition_function_data_getter.get_data(
            second_species_name, temperature)
        return np.exp(fitted_ratios[1])*(partition_function_first_species/partition_function_second_species)

    def _get_ln(self, species_data, integrals):
        return (integrals*species_data[:, 0]*1E-7) / (species_data[:, 2]*species_data[:, 1])
