...
class AtomicLinesDataPreparator:
    def __init__(self, atomic_lines_fetcher: AtomicLinesFetcher, atomic_lines_parser: AtomicLinesParser) -> None:
        self.atomic_lines_fetcher = atomic_lines_fetcher
        self.atomic_lines_parser = atomic_lines_parser

    def prepare_atomic_lines_data(self, species_name: str, target_peaks: np.ndarray):
        lower_wavelength, upper_wavelength = self._get_wavelength_range(
            target_peaks)
    ...
    def _filter_data(self, parsed_data):
        ...
        colunm_and_row_filtered_data = column_filtered_data[column_filtered_data[NOT_NA_FILTER_COLUMN].notna(
        )]

        return colunm_and_row_filtered_data

    def _fetch_atomic_lines_data_from_nist(self, species_name, lower_wavelength, upper_wavelength):
        return self.atomic_lines_fetcher.fetch(species_name, lower_wavelength, upper_wavelength)
