...
def BPlot(peak_table, integrals_with_constants):
    ...
    ln = np.log(np.divide(np.multiply(peak_data[:,5], peak_data[:,1]*1E-7),np.multiply(h*c*peak_data[:,3], peak_data[:,2])))
    E = peak_data[:,4]
...
def atom_ion_ratio(ne, T, atom_peak_table, ion_peak_table, integrals_with_constants, Ionization):
    ...
    atom_ln = np.divide(np.multiply(atomdata[:,5], atomdata[:,1]*1E-7),np.multiply(h*c*atomdata[:,3], atomdata[:,2]))
    ion_ln = (np.divide(np.multiply(iondata[:,5], iondata[:,1]*1E-7),np.multiply(h*c*iondata[:,3], iondata[:,2])))
...
def SBPlot_classic(ne, T, atom_peak_table, ion_peak_table, integrals_with_constants, Ionization):
    atomdata = spec_data(atom_peak_table, integrals_with_constants)
    iondata = spec_data(ion_peak_table, integrals_with_constants)
    ln_atom = np.log(np.divide(np.multiply(atomdata[:,5], atomdata[:,1]*1E-7),np.multiply(h*c*atomdata[:,3], atomdata[:,2])))
    ln_ion = np.log(np.divide(np.multiply(iondata[:,5], iondata[:,1]*1E-7),np.multiply(h*c*iondata[:,3], iondata[:,2]))) - np.log(2*np.power(X,1.5)*np.divide(np.power(T,1.5),ne))
    ln = np.concatenate((ln_atom, ln_ion))
    E = np.concatenate((atomdata[:,4], iondata[:,4] + Ionization))
    return np.stack((E, ln), axis=-1 )

def SBLP(ne_init, T_init, atom_peak_table, ion_peak_table, integrals_with_constants, Ionization, max_iter, T_tolerance, ne_tolerance):
    T_values = np.zeros(max_iter)
    ne_values = np.zeros(max_iter)
    T_values[0] = T_init
    ne_values[0] = ne_init
    i = 0
    plot = atom_ion_ratio(ne_values[i], T_values[i], atom_peak_table, ion_peak_table, integrals_with_constants,  Ionization)
    T = T_fit(plot)[0]
...
###Input
##Filename containing the line integrals with constants
filename = "AuAgCu_15-15-70_2eV_1E19_10000res_integrals_with_constants"

##Peak table for calculations
CuI_species = np.array([465.11, 510.55, 515.32, 521.82])
CuII_species = np.array([ 495.36, 512.18,  518.34])

###Temperature calculations
integrals_with_constants = genfromtxt(filename+".txt")
...
print('Temperature from Boltzmann plot of Cu I lines:', BPlot_CuI_results[0], 'K', 'with an R2:', BPlot_CuI_results[2])
