...
def plot_detected_peaks(spectrum, peak_indices, wl_start, wl_end):
    ...
    intensity_min = (
        np.amin(spectrum[wl_start_idx:wl_end_idx, 1])
        - np.amax(spectrum[wl_start_idx:wl_end_idx, 1]) * 0.02
    )  # lower intensity limit for plots
...
def calculate(baseline_corrected_spectrum, height, wlen, output_filename=None):
    logging.info(
        f"""Calculating integrals using the baseline corrected spectrum and the following parameters:
    height:{height}
    wlen:{wlen}"""
    )
...
def main(config):
    if config["enabled"]:
        ...
        calculate(
            baseline_corrected_spectrum, height, wlen, output_filename=output_filename
        )
